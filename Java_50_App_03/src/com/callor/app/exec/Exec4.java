...
public class Exec4 {
	...
	public static void main(String[] args) {
		...
		// 소수는 1과 자기 자신만으로 나누어떨어지기때문에 반복문을 생성하고 내부에 if 조건문에서 1과 자기자신(num)을 배제하고
		// 계산하여 0으로 나누어떨어지면 그것은 합성수이고, 그렇지 않은 경우는 소수라는 결론

		// 소수를 판별하는 조건문 선언
		boolean pNum = false;
		
		// 1은 소수도 합성수도 아니고, 2인 경우에는 반복문이 실행되지 않기 때문에 2를 기본값으로 설정한다.
		// 1과 자기자신(num) 외에 다른 수로 나누어떨어질 경우(num % 1 == 0 이 true일 경우) 합성수이다.

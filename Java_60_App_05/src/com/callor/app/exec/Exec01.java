 ...
 * 그 양이 별로 많지 않기때문에 코딩으로 데이터를 정렬할 수 있다
 * 
 * insert 정렬, bubble 정렬, selection 정렬, Quick 정렬
 * merge 정렬, shell 정렬, Radix 정렬 알고리즘이 있다
 * 
 * 간단한 정렬코드를 이해하면 쉽게 정렬을 수행할 수 있다
 * 
 * insert 정렬과 bubble 정렬의 중간쯤 성능을 갖는 알고리즘
 * 이 알고리즘은 정식 정렬알고리즘으로 인정받지는 않는다
 * 
 * 전체 배열이나 리스트를 for문으로 반복하면서
 * 0번 위치와 1 ~ 끝위치까지 비교하면서 큰값은 오른쪽으로
 * 작은값은 왼쪽으로
 * 1번위치와 2 ~ 끝위치까지 비교하면서 큰값은 오른쪽으로
 * 작은값은 왼쪽으로
 * 
 *  이 순회(반복)를 전체 개수만큼 반복 실행하여
 *  데이터를 정렬하는 알고리즘이다
 *  코드가 매우 단순해서 조금만 살펴보면 쉽게 이해할 수 있다
 *  insert 정렬과 비교했을 때 비교하는 방법에서 조금 차이가 있다
 *  큰 데이터가 아닐 경우는 insert 정렬에 비해 조금 빠르다
 */
public class Exec01 {
